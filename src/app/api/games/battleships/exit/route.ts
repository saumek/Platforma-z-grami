...
import { requestBattleshipExit, respondBattleshipExit } from "@/lib/battleships";
...
export async function POST(request: Request) {
  try {
    ...
    const result =
      body.action === "respond"
        ? await respondBattleshipExit(user.currentRoomCode, session.user.id, Boolean(body.approve))
        : await requestBattleshipExit(user.currentRoomCode, session.user.id);
